...
export default function tabs(server) {
    ...
    server.get('/requests/sent', (req, res, next) => {
        send(req, 'src/views/sent_requests.html').pipe(res);
    });

    server.get('/requests/received', (req, res, next) => {
        send(req, 'src/views/received_requests.html').pipe(res);
    });
